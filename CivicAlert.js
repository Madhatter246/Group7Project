...
/*Initialize Demo Data*/
    function initializeDemoData() {
        ...
        systemData.nextIncidentId = systemData.incidents.length + 1;
    }
        ...
        <div class="incident-card">
            <div class="incident-header">
                ...
                <div>
                    ...
                    ${incident.responding ? '<span class="incident-badge badge-responding">EN ROUTE</span>' : ''}
                </div>
            </div>
...
function updateAdminIncidents() {
    ...
    const sortValue = document.getElementById('admin-sort')?.value || 'newest';

    /*FILTER*/
    let filtered = systemData.incidents.filter(incident => {
        return (
            incident.type.toLowerCase().includes(searchValue) ||
            incident.location.toLowerCase().includes(searchValue) ||
            incident.reportedBy.toLowerCase().includes(searchValue)
        );
    });
    ...
    filtered.sort((a, b) => {
        ...
        if (sortValue === 'severity-high') {
            return (severityOrder[b.damageLevel] || 0) - (severityOrder[a.damageLevel] || 0);
        }

        if (sortValue === 'severity-low') {
            return (severityOrder[a.damageLevel] || 0) - (severityOrder[b.damageLevel] || 0);
        }
...
function respondToIncident(incidentId) {
    ...
    if (incident) {
        incident.responding = true;

        showMessage(`Responder is on the way to ${incident.location}`, 'success');
        updateDisplay();
    }
}
...
document.getElementById('citizen-form').addEventListener('submit', function(e) {
        e.preventDefault();
        const id = systemData.nextIncidentId++;
        const newIncident = {
            id:id,
...
window.addEventListener("load", function () {
  ...
  window.addEventListener("load", function () {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', updateAdminIncidents);
    }

    if (sortSelect) {
        sortSelect.addEventListener('change', updateAdminIncidents);
    }
});
  initializeDemoData();
  updateDisplay();
